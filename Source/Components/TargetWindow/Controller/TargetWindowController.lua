...
function CustomUI.TargetWindow.Tab.OnShown(contentName)
...
function CustomUI.TargetWindow.Tab.OnToggleEnable()
    local newState = not CustomUI.IsComponentEnabled("TargetWindow")
    CustomUI.SetComponentEnabled("TargetWindow", newState)
    ButtonSetPressedFlag(SystemData.ActiveWindow.name, newState)
end

function CustomUI.TargetWindow.Tab.OnFilterChanged()
    CustomUI.BuffFilterSection.OnFilterChanged(
        function() return CustomUI.TargetWindow.GetBuffFilterHostile() end,
        function()
            local s = CustomUI.TargetWindow.GetSettings()
